...
def generate_parabolic_expansion(r_start, r_end, x_start, length, curvature=1.0, num_points=100):
    ...
    a = (r_end - r_start) / (length**2)  # 抛物线系数
...
class NozzleGUI:
    def __init__(self, master):
        ...
        # 嵌入 Matplotlib 图形（设置黑色背景）
        self.figure, self.ax = plt.subplots(figsize=(6, 6), facecolor="black")
        ...
        for i, (label_text, default, cast_type, tooltip_text) in enumerate(param_list):
            label = tk.Label(self.right_frame, text=label_text, bg="black", fg="white")
            label.grid(row=i, column=0, sticky="w", padx=5, pady=2)
            var = tk.StringVar(value=default)
            entry = tk.Entry(self.right_frame, textvariable=var, bg="black", fg="white", insertbackground="white")
        ...
        # 文本框显示 Mach 数计算结果
        self.calc_text = tk.Text(self.right_frame, height=15, width=40, bg="black", fg="white", insertbackground="white")
        self.calc_text.grid(row=len(param_list), column=0, columnspan=2, padx=5, pady=5)
        self.calc_text.config(state="disabled")
        
        # 文本框显示警告信息
        self.warning_text = tk.Text(self.right_frame, height=5, width=40, bg="black", fg="white", insertbackground="white")
        self.warning_text.grid(row=len(param_list)+1, column=0, columnspan=2, padx=5, pady=5)
        self.warning_text.config(state="disabled")
        
        # “Generate File” 按钮
        self.stl_button = tk.Button(self.right_frame, text="Generate File", command=self.generate_file, bg="black", fg="white")
    ...
    def generate_preview(self):
        ...
        # 将原来黑色的线条改为白色
        self.ax.plot(x_inner, r_inner, 'w-', lw=1)
        self.ax.plot(x_inner, r_outer, 'w-', lw=1)
        self.ax.plot(x_inner, -r_inner, 'w-', lw=1)
        self.ax.plot(x_inner, -r_outer, 'w-', lw=1)
        self.ax.axhline(0, color='white', lw=0.8)
